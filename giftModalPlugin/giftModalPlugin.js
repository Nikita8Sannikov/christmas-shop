export const giftModalPlugin = (gifts, card) => {
	...
	card.forEach((card, index) => {
		...
		card.addEventListener("click", (event) => {
			...
			modalSuperpowers.innerHTML = `<h5>ADDS SUPERPOWERS TO:</h5>`;
				...
				superPower.innerHTML = `
                        ...
                        <div class="superPower-range">
                            <div class="superPower-range-number">${value}</div>
                             <div class="superPower-range-bar" >
							 ${Array.from({ length: 5 })

									.map((_, index) => {
										const opacity =
											index < Math.floor(value / 100)
												? 1
												: 0.1;
										return `
									<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style="opacity: ${opacity}">
									<g clip-path="url(#clip0_51177_26)">
